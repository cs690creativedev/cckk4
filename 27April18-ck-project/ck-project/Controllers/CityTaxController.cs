...
{
    public class CityTaxController : Controller
    {
        ...
        public ActionResult CityTaxNew(int? page, string search = null, string Type = null, string Type1 = null, string Start = null, string end = null, string msg = null)
        {
            ...
            try
            {
                ...
                {
                    var result = db.taxes
                        .Where(l => (l.tax_anme == "City Tax" && l.deleted == false))
                ...
                else
                {
                    ...
                    {
                        var result = db.taxes
                        .Where(l => (l.tax_anme == "City Tax" && l.state == Type && l.deleted == false && l.city == Type1))
                        .OrderBy(l => l.state).ThenBy(l => l.city).ThenBy(l => l.start_date)
                        .ToList();
                    ...
                    {
                        var result = db.taxes
                            .Where(l => (l.tax_anme == "City Tax" && l.state == Type && l.deleted == false))
                            .OrderBy(l => l.state).ThenBy(l => l.city).ThenBy(l => l.start_date)
                            .ToList();
        ...
        public ActionResult Add(FormCollection form)
        {
            try
            {
                ...
                if (db.taxes.Any(p => p.state == target.state && p.tax_anme == "City Tax" && p.city== target.city && p.deleted==false))
                {
                    ViewBag.m = "Duplicate City Tax.";
                    return View();
                }
                else
                {
                    var ctx1 = new ckdatabase();
                    query = "Select Top 1 tax_number from taxes where tax_anme = 'City Tax' and state = '" + target.state + "' and city='" + target.city + "' and start_date<'" +
                        target.start_date + "' and deleted=0 order by start_date Desc";

                    PrevId = ctx1.Database.SqlQuery<int>(query).FirstOrDefault<int>();
                    if (PrevId != 0)
                    {
                        query = "Update taxes set end_date='" + dt1 + "' where tax_anme='City Tax' and state='" + target.state + "' and city='" + target.city + "'";
                    ...
                    db.taxes.Add(target);
                    db.SaveChanges();
                    ViewBag.m = "The City Tax was successfully created " + "on " + System.DateTime.Now;
                    string search = null;
                    return RedirectToAction("CityTaxNew", new { search, msg = ViewBag.m });
                }
            }
            catch (Exception e)
            {
                ViewBag.m = "The City Tax was not created " + e.Message;
                return View();
            }
        }
        ...
        public ActionResult Delete(int id)
        {
            try
            {
                List<tax> CityTax_list = db.taxes.Where(d => d.tax_number == id).ToList();
                ViewBag.Customerslist = CityTax_list;
                tax target = CityTax_list[0];
                target.deleted = true;
                db.SaveChanges();
                ViewBag.m = "The City Tax was successfully deleted.";
                string search = null;
                return RedirectToAction("CityTaxNew", new { search, msg = ViewBag.m });
            }
            ...
            {
                ViewBag.m = "The City Tax was not deleted ..." + e.Message;
                string search = null;
                return RedirectToAction("CityTaxNew", new { search, msg = ViewBag.m });
            }
        }
        ...
        public ActionResult Edit(int id, FormCollection fo)
        {
            try
            {
                ...
                var ctx1 = new ckdatabase();
                query = "Select Top 1 tax_number from taxes where tax_anme = 'City Tax' and state = '" + target.state + "' and start_date<'" + target.start_date +
                ...
                ViewBag.m = " The City tax was successfully updated " + " on " + System.DateTime.Now;
                //return View(target);
